...
$str += 'Analyze', 'Test', 'CoverageReport'
$str += 'CreateHelpStart'
$str2 = $str
$str2 += 'Build', 'Archive'
...
Enter-Build {
    ...
    $script:ModuleSourcePath = [System.IO.Path]::Combine($BuildRoot, $script:ModuleName)
    $script:ModuleFiles = [System.IO.Path]::Combine($script:ModuleSourcePath, "*")

    $script:ModuleManifestFile = [System.IO.Path]::Combine($script:ModuleSourcePath, "$($script:ModuleName).psd1")
    $script:ModuleFile = [System.IO.Path]::Combine($script:ModuleSourcePath, "$($script:ModuleName).psm1")

    Set-ModuleFunctions -Name $ModuleFile
    ...
    $Script:ConfigsPath = [System.IO.Path]::Combine($BuildRoot, 'Config')
    $script:TestsPath = [System.IO.Path]::Combine($BuildRoot, 'Tests')
    $script:UnitTestsPath = [System.IO.Path]::Combine($script:TestsPath, 'Unit')
    $script:IntegrationTestsPath = [System.IO.Path]::Combine($script:TestsPath, 'Integration')

    $script:ArtifactsPath = [System.IO.Path]::Combine($BuildRoot, 'Artifacts')
    $script:ArchivePath = [System.IO.Path]::Combine($BuildRoot, 'Archive')

    $script:BuildModuleRootFile = [System.IO.Path]::Combine($script:ArtifactsPath, "$($script:ModuleName).psm1")
...
Set-BuildHeader {
    param($Path)
    # separator line
    Write-Build DarkMagenta ('=' * 90)

    # default header + synopsis
    #Write-Build Cyan "  Task: $Path"
    #Write-Build DarkCyan "   $(Get-BuildSynopsis $Task)"

    Write-Build Cyan "`t$($Task.Name.replace("_"," ").ToUpper())"
    Write-Build DarkCyan "`t$(Get-BuildSynopsis $Task)"
    "-" * 79
    ...
    Write-Build DarkYellow "   Manifest File   : $script:ModuleManifestFile"
    Write-Build DarkYellow "   Manifest Version: $($ManifestInfo.ModuleVersion)"
} #Set-BuildHeader
...
Add-BuildTask TestModuleManifest -Before ImportModuleManifest {
    ...
    Assert-Build (Test-Path -Path $script:ModuleManifestFile) 'Unable to locate the module manifest file.'
...
Add-BuildTask Test {
    ...
    if (Test-Path -Path $script:UnitTestsPath) {
        ...
        $pesterConfiguration.CodeCoverage.OutputFormat = 'Cobertura' # JaCoCo, CoverageGutters, Cobertura (JaCoCo, default: JaCoCo)
        ...
        Write-Build Gray ("`n      ...CODE COVERAGE - CommandsExecutedCount: {0}" -f $testResults.CodeCoverage.CommandsExecutedCount)
        ...
        if ($testResults.CodeCoverage.NumberOfCommandsExecuted -ne 0) {
            ...
            if ($testResults.CodeCoverage.NumberOfCommandsMissed -gt 0) {
                Write-Build Yellow ('Failed to analyze "{0}" commands' -f $testResults.CodeCoverage.NumberOfCommandsMissed)
            }
            <#
            ...
            if ([Int]$coveragePercent -lt $coverageThreshold) {
                ...
                $Message = '      Covered {0}% of {1} analyzed commands in {2} files.' -f $coveragePercent, $testResults.CodeCoverage.CommandsAnalyzedCount, $testResults.CodeCoverage.FilesAnalyzedCount
                ...
                Write-Build Green ("`n      ...Pester Unit Tests Complete!")
            }
        }
        else {
...
Add-BuildTask DevCC {
    ...
    $pesterConfiguration.CodeCoverage.Enabled = $true
    $pesterConfiguration.CodeCoverage.Path = "$PSScriptRoot\$ModuleName"
    $pesterConfiguration.CodeCoverage.RecursePaths = $true
    $pesterConfiguration.CodeCoverage.CoveragePercentTarget = $script:coverageThreshold
...
# Synopsis: Generate a coverage report using ReportGenerator
Add-BuildTask CoverageReport {

    Write-Build White '      Generating code coverage report...'

    $coverageFile = Join-Path -Path $script:ArtifactsPath -ChildPath 'ccReport\CodeCoverage.xml'
    $reportDir = '..\Coverage'
    ...
    if (Test-Path -Path $coverageFile) {
        ...
        if (-not (Get-Command reportgenerator -ErrorAction SilentlyContinue)) {
            ...
            #Write-Build Yellow '      ReportGenerator not found in PATH. Skipping.'
        }

        #else {
            try {
                reportgenerator "-reports:$coverageFile" "-targetdir:$reportDir" "-reporttypes:Html;MarkdownSummary;MarkdownAssembliesSummary;MarkdownSummaryGithub;MarkdownDeltaSummary;Cobertura"
                Write-Build Green '      ...Coverage report generated!'
            }
            catch {
                Write-Build Red "      Failed to generate coverage report: $($_.Exception.Message)"
            }
        #}
    }
    else {
        Write-Build Yellow '      Coverage xml file not found.'
    }
} #CoverageReport
...
Add-BuildTask CreateMarkdownHelp -After CreateHelpStart {
    ...
    if ($MissingDocumentation.Count -gt 0) {
        ...
        $Message = 'Missing documentation. Please review and rebuild.'
        Write-Build Red "  $Message !!`n"
        #TODO: Skip einbauen.
        #throw $Message
    }
    ...
    if ($fSynopsisOutput) {
        ...
        $Message = 'SYNOPSIS information missing. Please review.'
